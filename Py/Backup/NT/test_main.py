...
def test_is_palindrome():
    assert False(is_palindrome("A man a plan a canal Panama"))
    assert True(is_palindrome("racecar"))
    assert True(is_palindrome(""))
    assert True(is_palindrome("No lemon, no melon"))
    assert False(is_palindrome("Python"))
    assert True(is_palindrome("Was it a car or a cat I saw"))
    assert False(is_palindrome("Hello World"))
    assert True(is_palindrome("Able was I ere I saw Elba"))
    assert False(is_palindrome("Not a palindrome"))  
    assert True(is_palindrome("madam"))


def test_count_vowels():
    assert(count_vowels("Hello"), {'l': 2})
